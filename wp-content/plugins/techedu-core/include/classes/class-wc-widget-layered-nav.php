 ...
 */
class Techeduem_Widget_Layered_Nav extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = __( 'Display a list of attributes to filter products in your store.', 'techeduem' );
		$this->widget_id          = 'woocommerce_layered_nav_techeduem';
		$this->widget_name        = __( 'Techedu: Products Attribute', 'techeduem' );
		parent::__construct();
	}
	...
	public function init_settings() {
		...
		$this->settings = array(
			'title'        => array(
				'type'  => 'text',
				'std'   => __( 'Filter by', 'techeduem' ),
				'label' => __( 'Title', 'techeduem' ),
			),
			'attribute'    => array(
				'type'    => 'select',
				'std'     => '',
				'label'   => __( 'Attribute', 'techeduem' ),
				'options' => $attribute_array,
			),
			'display_type' => array(
				'type'    => 'select',
				'std'     => 'list',
				'label'   => __( 'Display type', 'techeduem' ),
				'options' => array(
					'list'     => __( 'List', 'techeduem' ),
					'dropdown' => __( 'Dropdown', 'techeduem' ),
				),
			),
			'query_type'   => array(
				'type'    => 'select',
				'std'     => 'and',
				'label'   => __( 'Query type', 'techeduem' ),
				'options' => array(
					'and' => __( 'AND', 'techeduem' ),
					'or'  => __( 'OR', 'techeduem' ),
				),
			),
		);
	...
	protected function layered_nav_dropdown( $terms, $taxonomy, $query_type ) {
		...
		if ( $taxonomy !== $this->get_current_taxonomy() ) {
			...
			$any_label      = apply_filters( 'woocommerce_layered_nav_any_label', sprintf( __( 'Any %s', 'techeduem' ), $taxonomy_label ), $taxonomy_label, $taxonomy );
			...
			if ( $multiple ) {
				echo '<button class="woocommerce-widget-layered-nav-dropdown__submit" type="submit" value="' . esc_attr__( 'Apply', 'techeduem' ) . '">' . esc_html__( 'Apply', 'techeduem' ) . '</button>';
			}
				...
				if ( jQuery().selectWoo ) {
					var wc_layered_nav_select = function() {
						jQuery( '.dropdown_layered_nav_" . esc_js( $taxonomy_filter_name ) . "' ).selectWoo( {
							...
							language: {
								noResults: function() {
									return '" . esc_js( _x( 'No matches found', 'enhanced select', 'techeduem' ) ) . "';
								}
							}
						} );
					};
	...
	protected function layered_nav_list( $terms, $taxonomy, $query_type ) {
		// List display.
		echo '<ul class="woocommerce-widget-layered-nav-list sidebar-list">';
		...
		foreach ( $terms as $term ) {
			...
			if ( $count > 0 || $option_is_set ) {
				...
				if( !empty( $color ) ){
					$term_html = '<a rel="nofollow" href="' . $link . '"><span class="color" style="background-color: '.$color.';">&nbsp;</span>' . esc_html( $term->name ) . '</a>';
				}else{
				...
				$link      = false;
				if( !empty( $color ) ){
					$term_html = '<span class="color" style="background-color: '.$color.';">&nbsp;</span><span>' . esc_html( $term->name ) . '</span>';
				}else{
